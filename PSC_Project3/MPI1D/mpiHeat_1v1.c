...
{
    switch (condition) {
        case 0: // MASTER -- North, West and East face
        ...
        case 1: // Middle processes -- West and East
            for (INT j = 0; j < nrow + nGhostLayers; j++) {
                for (INT i = 0; i < NX; i++) {
                    if (i == 0) {
                        phi[ IC ] = TMIN;
        ...
        case 2: // nProcs-1 process -- South West and East
            for (INT j = 0; j < nrow + nGhostLayers; j++) {
                for (INT i = 0; i < NX; i++) {
                    if (j == nrow + 1) {
                        phi[ IC ] = TMIN;
...
void outputMatrix(const REAL *phi)
{
    ...
    /* Printing to Screen for small solution visualization */
    for (INT j = 0; j < NY; j++) {
        for (INT i = 0; i < NX; i++) {
            printf("%6.2f ", phi[ IC ]);
        }
...
int main(int argc, char **argv)
{
    ...
    for (REAL iter = 0.f; iter < MAXITER; iter += DT) {
        // exchange_SendRecv(phi, src, dest, nrow, myRank); // Blocking SendRecv
        exchange_ISend_and_IRecv(phi, src, dest, nrow, myRank); // Nonblocking Isend and Irecv
